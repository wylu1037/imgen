...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return Response.json(
      { error: "Enter your API key in provider settings." },
      { status: 400 },
    );
  }
  ...
  if (!qualities.has(quality)) {
    return Response.json(
      { error: "Unsupported image quality." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (!b64 && !url) {
      return Response.json(
        {
          error:
            "The image was generated, but the provider did not return a displayable asset.",
        },
        { status: 502 },
      );
    }
    ...
    if (b64) {
      ...
      try {
        ...
        if (!upstream.ok) {
          throw new Error(
            `Upstream image fetch failed with status ${upstream.status}`,
          );
        }
    ...
    const cause = err.cause as
      | { code?: string; errno?: string; message?: string }
      | undefined;
    ...
    console.error("[/api/generate] image generation failed", {
      ...
      baseURLHost: baseURL ? new URL(baseURL).host : undefined,
    });
