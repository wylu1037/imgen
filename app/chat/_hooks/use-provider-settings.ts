...
function readStoredProviderSettings(): ProviderSettings | null {
  ...
  try {
    ...
    const legacyProvider = createProvider({
      ...
      models: [legacyDefaultModel, ...modelOptions.map((option) => option.value)],
...
export function useProviderSettings() {
  ...
  const syncSettings = React.useCallback(async (next: ProviderSettings) => {
    ...
    if (db) {
      await Promise.all(next.providers.map((provider) => db.upsertProvider(provider)))
  ...
  const setActiveProviderId = React.useCallback(
    (providerId: string) => {
      const next = settings.providers.some((provider) => provider.id === providerId)
  ...
  const saveProvider = React.useCallback(
    (provider: ProviderConfig) => {
      ...
      const exists = settings.providers.some((item) => item.id === normalized.id)
    ...
    (providerId: string) => {
      ...
      void (async () => {
        if (db) {
          ...
          await Promise.all(providers.map((provider) => db.upsertProvider(provider)))
